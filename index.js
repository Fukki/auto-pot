...
module.exports = function AutoPOT(mod) {
	...
	mod.hook('S_LOAD_TOPO', 3, e => {isBG = (e.zone === zoneBG); VehicleEx = null;});
		...
		if (config.enabled && config.mp) {
			if (e.target.equals(gameId)) {
				...
				for (let mp = 0; mp < mpPot.length; mp++) {
					if (!mpPot[mp][1].inCd && nowMP <= mpPot[mp][1].use_at && mpPot[mp][1].amount > 0 && isAlive && !isBG) {
