...
module.exports = function AutoPOT(mod) {
	...
	mod.hook('S_START_COOLTIME_ITEM', 1, e => {
		if (config.enabled) {
			let hp = hpPot.findIndex(item => s2n(item[0]) === e.item);
			if (hp >= 0) {
				hpPot[hp][1].inCd = true;
				clearTimeout(hpPot[hp][1].timeOut);
				hpPot[hp][1].timeOut = setTimeout(function (h) {
					hpPot[h][1].inCd = false;
				}, e.cooldown * 1000, hp);
			}
			let mp = mpPot.findIndex(item => s2n(item[0]) === e.item);
			if (mp >= 0) {
				mpPot[mp][1].inCd = true;
				clearTimeout(mpPot[mp][1].timeOut);
				mpPot[mp][1].timeOut = setTimeout(function (m) {
